...
image_path):
    pic_path = image_path
    mask_path = "/home/ee201814096/observations/mask_classifier/Data_Generator/images/blue-mask.png"
    show = False
    model = "hog"
...
class FaceMasker:
    KEY_FACIAL_FEATURES = ('top_lip', 'chin')
    ...
    def _mask_face(self, face_landmark: dict):
        ...
        lip_point = top_lip[len(top_lip) * 2 // 3]
        lip_v = np.array(lip_point)
        ...
        width_ratio = 1.2
        new_height = int(np.linalg.norm(lip_v - chin_bottom_v))

        # left
        mask_left_img = self._mask_img.crop((0, 0, width // 2, height))
        mask_left_width = self.get_distance_from_point_to_line(chin_left_point, lip_point, chin_bottom_point)
        mask_left_width = int(mask_left_width * width_ratio)
        mask_left_img = mask_left_img.resize((mask_left_width, new_height))

        # right
        mask_right_img = self._mask_img.crop((width // 2, 0, width, height))
        mask_right_width = self.get_distance_from_point_to_line(chin_right_point, lip_point, chin_bottom_point)
        ...
        # rotate mask
        angle = np.arctan2(chin_bottom_point[1] - lip_point[1], chin_bottom_point[0] - lip_point[0])
...
if __name__ == '__main__':
    #cli()
    image_path="/home/ee201814096/observations/experiements/data/without_mask"
